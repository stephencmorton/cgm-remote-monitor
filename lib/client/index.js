...
client.load = function load (serverSettings, callback) {
  ...
  $('.focus-range li').click(function(e) {
    ...
    if (li.attr('data-hours')) {
      ...
      $("#showhours").addClass('selected')
      var action = li.data('hours');
      if (action == "+")
      {
        client.zoom(1.2);
      }
  ...
  client.zoom = function zoom(factor)
  {
    var newrange = client.focusRangeMS;
    if (factor == 1)
    {
      newrange = newrange+1;
    }
    ...
    {
      newrange = Math.floor(client.focusRangeMS * factor);
    }
    newrange = Math.min(times.hours(24).msecs,newrange);
    newrange = Math.max(times.hours(1).msecs, newrange);
    client.focusRangeMS = newrange;
    var new_displayrange = Math.ceil(newrange/(60*60*1000));
    Storages.localStorage.set('focusHours', new_displayrange);
    refreshChart();
    $("#showhours").text(new_displayrange);
  }
//   $('#chartContainer').bind('mousewheel DOMMouseScroll', function(event){
//     //var hours = Storages.localStorage.get('focusHours');
