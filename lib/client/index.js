...
client.load = function load (serverSettings, callback) {
  ...
  $('#chartContainer').bind('mousewheel DOMMouseScroll', function(event){
    ...
    if (event.originalEvent.wheelDelta > 0 || event.originalEvent.detail < 0) {
      var newrange = Math.floor(client.focusRangeMS * 1.2);
      if (newrange > times.hours(24).msecs){newrange=times.hours(24).msecs;}
    }
    else {
      var newrange = Math.floor(client.focusRangeMS * 0.8);
      if (newrange < times.hours(2).msecs){newrange=times.hours(2).msecs;}
    }
    client.focusRangeMS = newrange;
    Storages.localStorage.set('focusHours', Math.ceil(newrange/(60*60*1000)));
    refreshChart();
});
